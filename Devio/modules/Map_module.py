...
class Entity(Object):
        ...
        if self.status == "jumping":
            ...
            for brick in bricks:
                    if collide(brick,self,x_offset):
                        self.y = self.y + 5
                        self.jump_velocity = 0
                        if brick.type == 'brick' and self.type == "player" and self.y +5 > brick.y + brick.img.get_height():
                            bricks.remove(brick)
                            score += 25
                            BREAK_BRICK_SOUND.play()
                        if brick.type == "spikeybrick" and self.type == 'player':
                            countdown = 0
                            DEATH_SOUND.play()
    ...
    def animate(self,timer):
        if self.type == 'player':
            if timer%15 == 0 and self.status == "stationary": #Stationary Animation
                self.img = DEVIO_SPRITE_SHEET_STANDING.get_image(int(timer/15),45,100)
                if self.direction == 'left':
                    self.img = pygame.transform.flip(self.img,True, False).convert_alpha()
            if timer%10 == 0 and self.status == "walking": #walking animation
                self.img = DEVIO_SPRITE_SHEET_WALKING.get_image(int(timer/10), 45,100)
                if self.direction == "left":
                    self.img = pygame.transform.flip(self.img,True, False).convert_alpha()
        ...
        if self.type == "turtle":
            if timer%15 == 0:
                self.img = TURTLE_SPRITE_SHEET.get_image(int(timer/15), 75,50)
                if self.direction == "right":
                    self.img = pygame.transform.flip(self.img,True, False).convert_alpha()
        if self.type == "jumperguy" and timer%10 == 0: #walking animation
                self.img = JUMPER_GUY_SPRITE_SHEET_WALKING.get_image(int(timer/10), 45,100)
                if self.direction == "left":
                    self.img = pygame.transform.flip(self.img,True, False).convert_alpha() 
