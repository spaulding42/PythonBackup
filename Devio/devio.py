...
# frame0 = SPRITE_SHEET.get_image(1, 75,50)
...
def game(level):
    ...
    for i in range(enemy_turtle_count):
        ...
        new_enemy = generate_enemy(random.randrange(700,5000),random.randrange(0,FLOOR-50),'turtle',-3)
    ...
    while run:
        ...
        if keys[pygame.K_a]: #MOVE LEFT                  (a)
            ...
            if player.direction != 'left':
                ...
                player.img = DEVIO_SPRITE_SHEET_STANDING.get_image(0,45,100)
                player.img = pygame.transform.flip(player.img,True, False).convert_alpha()
            x_offset += player.velocity
            for ground_block in ground:
        ...
        if keys[pygame.K_SPACE]: #  JUMP                  (spacebar)
            if player.grounded == True and jump_able == True:
        ...
        for ground_block in ground: 
            ...
            # if collide_x(ground_block,player,x_offset):
            #     player.y = player.y - 5
